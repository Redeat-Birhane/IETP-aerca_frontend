...
export default function TaxWorkers() {
  ...
  useEffect(() => {
    fetch("https://ietp-aerca-backend.onrender.com/users/tax_workers/", {
      method: "GET",
      credentials: "include",
    })
      .then(async (res) => {
  ...
  const handleSubmit = async (workerEmail) => {
    ...
    try {
      const res = await fetch(
        "https://ietp-aerca-backend.onrender.com/users/ask/",
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  if (loading)
    return <p style={{ textAlign: "center", marginTop: "50px" }}>Loading tax workers...</p>;
        ...
        {workers.map((worker) => (
          <div className="taxworker-card" key={worker.email}>
            <div className="taxworker-header">
              ...
              {worker.photo ? (
                <img
                  src={`https://ietp-aerca-backend.onrender.com${worker.photo}`}
                  alt={worker.username}
              ...
              {/* Secret hidden fallback for onError events */}
